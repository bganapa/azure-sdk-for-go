...
// Delete delete resource group. This method may poll for completion. Polling can be canceled by passing the cancel
...
//
// resourceGroupName is the name of the resource group to be created or updated. parameters is parameters supplied to
// the export template resource group operation.
...
// on the operation. expand is the $expand query parameter top is query parameters. If null is passed returns all
// resource groups.
...
func (client GroupsClient) ListResourcesComplete(resourceGroupName string, filter string, expand string, top *int32, cancel <-chan struct{}) (<-chan GenericResource, <-chan error) {
	...
	go func() {
		...
		list, err := client.ListResources(resourceGroupName, filter, expand, top)
		if err != nil {
			errChan <- err
			return
		}
		...
		for list.NextLink != nil {
			list, err = client.ListResourcesNextResults(list)
			if err != nil {
				errChan <- err
				return
			}
